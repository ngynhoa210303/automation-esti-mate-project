...
dotenv.config();
test.skip("CI01: Create Item with Preliminaries And General section ", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[0].name, 0);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[0].name,
                dataSection[0].material_rate,
                dataSection[0].part_no,
                dataSection[0].labour_unit_rate_hour,
                dataSection[0].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[0].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[0].name);
