...
dotenv.config();
test.skip("CI11: Create Item MATV", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[10].name, 10);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[10].name,
                dataSection[10].material_rate,
                dataSection[10].part_no,
                dataSection[10].labour_unit_rate_hour,
                dataSection[10].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[10].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[10].name);
