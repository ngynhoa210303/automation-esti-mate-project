...
dotenv.config();
test.skip("CI05: Create Item Installation Of Lighting", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[4].name, 4);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[4].name,
                dataSection[4].material_rate,
                dataSection[4].part_no,
                dataSection[4].labour_unit_rate_hour,
                dataSection[4].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[4].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[4].name);
