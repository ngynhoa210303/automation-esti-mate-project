...
dotenv.config();
test.skip("CI12: Create Item Fire System", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[11].name, 11);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[11].name,
                dataSection[11].material_rate,
                dataSection[11].part_no,
                dataSection[11].labour_unit_rate_hour,
                dataSection[11].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[11].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[11].name);
