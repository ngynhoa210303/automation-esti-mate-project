...
dotenv.config();
test.skip("CI07: Create Item Installation Of Power", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[6].name, 6);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[6].name,
                dataSection[6].material_rate,
                dataSection[6].part_no,
                dataSection[6].labour_unit_rate_hour,
                dataSection[6].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[6].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[6].name);
