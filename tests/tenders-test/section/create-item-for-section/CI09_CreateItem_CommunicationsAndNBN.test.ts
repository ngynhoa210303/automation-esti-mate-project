...
dotenv.config();
test.skip("CI09: Create Item Communications & NBN", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[8].name, 8);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[8].name,
                dataSection[8].material_rate,
                dataSection[8].part_no,
                dataSection[8].labour_unit_rate_hour,
                dataSection[8].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[8].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[8].name);
