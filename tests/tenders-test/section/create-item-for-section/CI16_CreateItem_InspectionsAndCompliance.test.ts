...
dotenv.config();
test.skip("CI16: Create Item Inspections And Compliance", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[15].name, 15);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[15].name,
                dataSection[15].material_rate,
                dataSection[15].part_no,
                dataSection[15].labour_unit_rate_hour,
                dataSection[15].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[15].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[15].name);
