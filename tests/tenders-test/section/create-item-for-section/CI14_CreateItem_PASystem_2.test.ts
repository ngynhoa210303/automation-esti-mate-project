...
dotenv.config();
test.skip("CI14: Create Item PASystem 2", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[13].name, 13);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[13].name,
                dataSection[13].material_rate,
                dataSection[13].part_no,
                dataSection[13].labour_unit_rate_hour,
                dataSection[13].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[13].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[13].name);
