...
dotenv.config();
test.skip("CI04: Create Item Cable Trays", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[3].name, 3);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[3].name,
                dataSection[3].material_rate,
                dataSection[3].part_no,
                dataSection[3].labour_unit_rate_hour,
                dataSection[3].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[3].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[3].name);
