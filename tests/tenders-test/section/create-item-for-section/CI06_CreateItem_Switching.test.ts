...
dotenv.config();
test.skip("CI06: Create Item Switching", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[5].name, 5);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[5].name,
                dataSection[5].material_rate,
                dataSection[5].part_no,
                dataSection[5].labour_unit_rate_hour,
                dataSection[5].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[5].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[5].name);
