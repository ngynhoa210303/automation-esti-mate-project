...
dotenv.config();
test.skip("CI03: Create Item Conduit", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[2].name, 2);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[2].name,
                dataSection[2].material_rate,
                dataSection[2].part_no,
                dataSection[2].labour_unit_rate_hour,
                dataSection[2].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[2].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[2].name);
