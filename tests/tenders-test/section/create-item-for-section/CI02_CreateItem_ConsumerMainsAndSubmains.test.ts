...
dotenv.config();
test.skip("CI02: Create Item Consumer Mains And Submains", () => {
    ...
    test("Create Item", async ({ page }) => {
        ...
        const createItem = new CreateItem(page, dataSection[1].name, 1);
        ...
        await test.step('Create item', async () => {
            await createItem.fillToInformation(
                dataSection[1].name,
                dataSection[1].material_rate,
                dataSection[1].part_no,
                dataSection[1].labour_unit_rate_hour,
                dataSection[1].labour_unit_rate_mins,
            );
        })
        await test.step('Check select element after create', async () => {
            await createItem.selectClick(dataSection[1].name);
            await createItem.hideClick();
        })
        await test.step('Clear data', async () => {
            const search = new SearchItemOfFactory(page);
            await search.search(dataSection[1].name);
